...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Обновление налогового рейта города
  app.post('/api/cities/:cityId/tax', async (req, res) => {
    try {
      const { cityId } = req.params;
      const { taxRate } = req.body;

      // Проверяем, что налоговый рейт валидный
      if (taxRate === undefined || taxRate < 0 || taxRate > 10) {
        return res.status(400).json({ error: "Invalid tax rate" });
      }

      // Обновляем город
      await storage.updateCity(parseInt(cityId), { taxRate });

      // Получаем обновленный город
      const updatedCity = await storage.getCity(parseInt(cityId));
  ...
  // Перемещение армии между городами
  app.post('/api/cities/:fromCityId/transfer/:toCityId', async (req, res) => {
    try {
